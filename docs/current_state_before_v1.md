...
This repository was initially created at `C:\Users\wangz\vla-score-tail` and later renamed to `C:\Users\wangz\score-tail vla`. At the start of v1, only the requested directory scaffold existed: `docs/`, `paper/`, `src/vla_tailguard_audit/`, `experiments/`, `scripts/`, `tests/`, `results/`, and `results/figures/`.

Nearby score-tail repositories exist for WAM, JEPA, and diffusion-policy projects. They can inform lightweight command and packaging conventions only. They must not determine this repository's scientific object or claims.

## What Is Missing

- Exact finite tie-aware score-tail implementation and tests.
...
1. Implement the exact finite tie-aware score-tail law and validate it empirically.
